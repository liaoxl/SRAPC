...
switch(segOpts.posteriorMethod)
  ...
  case 'gmm_bs_fg'
    posteriorImage=do_gmmBS_posteriorImage_fgGmm(features,labelImg,segOpts);
  case 'none'
...
function posteriors=do_gmmBS_posteriorImage_fgGmm(features,labelImg,segOpts)
import gsc.gmm.*;
% --- learn the gmm first --------
fgFeatures=features(:,labelImg(:)==1);
fgGmm=init_gmmBS(fgFeatures,segOpts.gmmNmix_fg);
...
% --- Now compute posteriors ------
posteriors=compute_gmmPosteriors_byfgGmm(features,fgGmm,segOpts.gmmLikeli_gamma,segOpts.gmmUni_value);
posteriors=reshape(posteriors,size(labelImg));
